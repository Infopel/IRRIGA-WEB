...
namespace IRRIGA.Controllers
{
    public class AdministrationController : Controller
    {
        ...
        public AdministrationController(UserManager<IdentityUser> userManager,
                                      ...
                                      IRegadiosRepository regadiosRepository)
        {
        ...
        public async Task<IActionResult> CreateUserWithRole(CreateUserWithRoleViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _userManager.CreateAsync(user, "irriga@2022");
                ...
                if (result.Succeeded)
                {
                    ...
                    IEnumerable<Usuario> usuarios = _usuariosRepository.GetAll();
                    ...
                    Usuario newUser = new Usuario()
                    {
                        ...
                        IdForMobile = usuarios.Count() + 1
                    };

                    await _usuariosRepository.Create(newUser);

                    if (model.IdRegadios.Count() != 0)
                    {
                        foreach (var regadios in model.IdRegadios)
                        {
                            UsuarioRegadio newUserRegadios = new UsuarioRegadio()
                            {
                                IdUsuario = newUser.Id,
                                IdRegadio = Guid.Parse(regadios),
                                IsRemoved = false,
                            };

                            await _usuariosRepository.CreateUsuarioRegadio(newUserRegadios);
                        }
                    }

                    if (model.IdInqueritos.Count() != 0)
                    {
                        foreach (var inqueritos in model.IdInqueritos)
                        {
                            UsuarioInquerito newUserInquerito = new UsuarioInquerito()
                            {
                                IdUsuario = newUser.Id,
                                IdInquerito = Guid.Parse(inqueritos),
                                IsRemoved = false,
                            };

                            await _usuariosRepository.CreateUsuarioInquerito(newUserInquerito);
                        }
                    }

                    if (model.IdEscolas.Count() != 0)
                    {
                        foreach (var escolas in model.IdEscolas)
                        {

                            UsuarioEscola newUserEscolas = new UsuarioEscola()
                            {
                                IdUsuario = newUser.Id,
                                IdEscola = Guid.Parse(escolas),
                                IsRemoved = false,
                            };

                            await _usuariosRepository.CreateUsuarioEscola(newUserEscolas);

                        }
                    }

                    if(model.IdAssociacaoes.Count() != 0)
                    {
                        foreach (var associacoes in model.IdAssociacaoes)
                        {
                            UsuarioAssociaco newUserAssociacoes = new UsuarioAssociaco()
                            {
                                IdUsuario = newUser.Id,
                                IdAssociacao = Guid.Parse(associacoes),
                                IsRemoved = false,
                            };

                            await _usuariosRepository.CreateUsuarioAssociacao(newUserAssociacoes);
                        }
                    }
        ...
        public async Task<Guid> GetReagdioGuidIdBy(int idForMobile)
        {
